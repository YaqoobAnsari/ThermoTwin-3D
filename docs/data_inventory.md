...
|---|---|---|---|---|---|
| **TUM2TWIN CityGML** | `data/raw/tum2twin-datasets/` (5.0 GB) — **27 LoD2** + **27 LoD3** `.gml` | real geometry | TUM2TWIN terms | **Two real-geometry operator columns:** LoD2 → `real-CityGML` (Exp 2.6); LoD3 → `real-CityGML-LoD3`, the higher-fidelity column — tessellated shells (≤69k tris) **coplanar-merged** to planar faces (`geometry/coplanar.py`) then the same per-surface FV. Both gated by tests. | LoD2 = coarse shells (no window holes); LoD3 photogrammetric, merged faces are bbox-rectangular approximations; 1/27 LoD3 files truncated (skipped) |
| **TUM2TWIN thermal orthophoto** | `data/raw/tum2twin-datasets/tif/TUM_flipped_georeferenced.tif` (2.3 MB) — georef GeoTIFF, EPSG:25832, 4 m/px, 3-band tone-mapped | real thermal | TUM2TWIN terms | **Measured-thermal cross-task rung** (`data/tum2twin_tir.py`): register CityGML footprints onto the IR ortho (stdlib GeoTIFF reader `geometry/geotiff.py`), score heat-loss-anomaly **footprint enrichment** — **2.11×** over 27 envelopes. The only rung scored on *real* IR, not sim physics. | Tone-mapped → **relative** thermal, not °C; ~42 % valid coverage; nadir view (roofs dominate) |
